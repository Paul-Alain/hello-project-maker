          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border-2 border-black/10 bg-card p-4 shadow-md">
              ...
              <p className="mb-4 text-xs text-muted-foreground">
                Période :{" "}
                {filterMode === "month-year" ? `${MONTHS[selMonth - 1]} ${selYear}` : `${customStart} → ${customEnd}`}
                ...
                >
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs font-medium text-amber-700">
                      Dans environ {hoursLeft}h{r.balance > 0 && ` · Solde restant : ${money(r.balance)}`}
                    </p>
                  </div>
                  <Button
                    size="sm"
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-black/20">
        <div className="h-full rounded-full bg-white/80 transition-all" style={{ width: `${pctOccupied}%` }} />
      </div>
...
function AggregateCard({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div
      className="rounded-2xl bg-amber-800 p-5 text-white"
